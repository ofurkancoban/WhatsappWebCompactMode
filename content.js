...
async function syncColorImmediately(img) {
    ...
    if (color) {
        ...
        if (match && match.length === 3) {
            ...
            // Set the background tint (using user's chosen 0.3 opacity for most refined glass effect)
            document.body.style.setProperty('--waw-doodle-layer', `rgba(${r}, ${g}, ${b}, 0.3)`);
        }
    } else {
        document.body.style.setProperty('--waw-doodle-layer', 'transparent');
        document.body.classList.remove('waw-bg-light', 'waw-bg-dark');
...
function cleanOriginalButtonStyle(el) {
    // Bu fonksiyon artık kullanılmıyor, ileride ihtiyaç olabilir diye duruyordu.
    // Ancak temiz kod prensibi gereği kaldırılabilir veya içi boşaltılabilir.
}
...
function syncChatHeaderBackground() {
    ...
    if (avatar && avatar.src) {
        ...
        if (header.style.getPropertyValue('--header-bg-image') !== bgUrl) {
            ...
            // Chat değişince arkaplan rengini de GÜNCELLE
            syncColorImmediately(avatar);
        }
    } else {
        if (header.style.getPropertyValue('--header-bg-image') !== '') {
            header.style.removeProperty('--header-bg-image');
            // Eğer resim yoksa arkaplanı sıfırla
            syncColorImmediately(null);
        }
    }
}
...
function initEventListeners() {
    ...
    window.addEventListener('pointerdown', (e) => {
        ...
        if (row) {
            ...
            // 3. Header ve TopBar gecikmesini önlemek için tıklar tıklamaz sync'leri çalıştır
            setTimeout(() => {
                syncCustomTopBar();
                syncChatHeaderBackground();
            }, 5);
            ...
            setTimeout(() => {
                syncCustomTopBar();
                syncChatHeaderBackground();
            }, 150); // Fallback for delayed loads
        }
    }, { capture: true, passive: true });
